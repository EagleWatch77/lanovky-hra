...
import {
  ...
  KONKURENCIA_VYVIJA_SA,
  cielovaUrovenKonkurencie,
  urovenPodlaHraca,
  KONKURENCIA_PAUZA_DNI,
...
import { lanovkovyMultiplikatorDna, parkoviskovyMultiplikatorDna, zasnezovanieFunguje, jeBurkaDnes, jeSilnyVietorDnes, typPocasiaDnes } from "./pocasie";
...
export async function spracujStanicu(supabase, stanicaId) {
  ...
  // --- Vývoj konkurencie ---
  // Konkurencia rastie, keď stredisko priláka viac ľudí, alebo keď ju hráč predbehne kvalitou.
  const dennyPocetPreKonkurenciu = st.denny_pocet_turistov || 0;
  for (const k of konkurenciaData) {
    // Dokončenie rozostavanej prestavby
    if (k.prestavba_koniec && new Date(k.prestavba_koniec) <= teraz) {
      const novaUroven = (k.uroven || 1) + 1;
      await supabase
        .from("konkurencia")
        .update({ uroven: novaUroven, prestavba_koniec: null, posledna_prestavba: teraz.toISOString() })
        .eq("id", k.id);
      k.uroven = novaUroven;
      k.prestavba_koniec = null;
      k.posledna_prestavba = teraz.toISOString();
      continue;
    }

    if (k.stav !== "hotovo") continue;
    if (k.prestavba_koniec) continue; // už prestavuje
    if (!KONKURENCIA_VYVIJA_SA.includes(k.kategoria)) continue;

    const aktualna = k.uroven || 1;
    if (aktualna >= 3) continue;

    // Pauza medzi prestavbami — najviac raz za herný rok
    const odPoslednej = k.posledna_prestavba
      ? (teraz - new Date(k.posledna_prestavba)) / (1000 * 60 * 60 * 24)
      : 999;
    if (odPoslednej < KONKURENCIA_PAUZA_DNI) continue;

    const podlaTrhu = cielovaUrovenKonkurencie(dennyPocetPreKonkurenciu);
    const podlaHraca = urovenPodlaHraca(hotoveTeraz, k.kategoria);
    const cielova = Math.max(podlaTrhu, podlaHraca);

    // Prestavuje sa vždy len o jeden stupeň a trvá to rovnako ako pôvodná stavba
    if (cielova > aktualna) {
      const dni = vystavbaVRealnychDnoch(KONKURENCIA_VYSTAVBA_MESIACOV[k.kategoria] || 2);
      const koniecPrestavby = new Date(teraz.getTime() + dni * 24 * 60 * 60 * 1000);
      await supabase
        .from("konkurencia")
        .update({ prestavba_koniec: koniecPrestavby.toISOString() })
        .eq("id", k.id);
      k.prestavba_koniec = koniecPrestavby.toISOString();
    }
  }

  const pocetKonkurencie = {};
  const urovneKonkurencie = {};
  for (const k of konkurenciaData) {
    if (k.stav === "hotovo") {
      const kluc = `${k.zona}:${k.kategoria}`;
      pocetKonkurencie[kluc] = (pocetKonkurencie[kluc] || 0) + 1;
      urovneKonkurencie[kluc] = Math.max(urovneKonkurencie[kluc] || 1, k.uroven || 1);
    }
  }
...
const globalnyMult = globalnyCenovyMultiplikator(st, hotoveTeraz);
  ...
  // Raz za herný týždeň nastaví hra ceny sama, ak má hráč zaplatenú automatiku.
  const autoAktivne = st.auto_ceny_do && new Date(st.auto_ceny_do) > teraz;
  if (autoAktivne) {
    const hodinOdPoslednej = st.auto_ceny_posledne
      ? (teraz - new Date(st.auto_ceny_posledne)) / (1000 * 60 * 60)
      : 999;

    if (hodinOdPoslednej >= CENA_COOLDOWN_HODIN) {
      const tyzdenIndex = Math.floor(teraz.getTime() / (CENA_COOLDOWN_HODIN * 60 * 60 * 1000));
      const maZasnezovanieTeraz = hotoveTeraz.some((b) => b.kategoria === "zasnezovanie");

      // Skipas — zvlášť zimný a letný
      const refSkipasZima = referencnaCenaSkipasu(hotoveTeraz, hDatum, globalnyMult, maZasnezovanieTeraz);
      const noveCeny = {};
      if (refSkipasZima) {
        noveCeny.cena_skipasu = autoCena(st.id, "lanovka", "skipas", tyzdenIndex, refSkipasZima);
        noveCeny.cena_skipasu_leto = Math.max(1, Math.round(noveCeny.cena_skipasu * 0.45));
      }

      // Parkovné, penzióny, hotely
      const refPark = skutocnaReferencnaCena("parkovisko", "parkovisko_asfaltove", hDatum, globalnyMult);
      if (refPark) {
        noveCeny.cena_parkovne_luka = autoCena(st.id, "parkovisko", "luka", tyzdenIndex, refPark);
        noveCeny.cena_parkovne_udolie = autoCena(st.id, "parkovisko", "udolie", tyzdenIndex, refPark);
      }
      const refPenzion = skutocnaReferencnaCena("penzion", "penzion", hDatum, globalnyMult);
      if (refPenzion) noveCeny.cena_penzion = autoCena(st.id, "penzion", "penzion", tyzdenIndex, refPenzion);
      const refHotel = skutocnaReferencnaCena("hotel", "hotel_3", hDatum, globalnyMult);
      if (refHotel) noveCeny.cena_hotel = autoCena(st.id, "hotel", "hotel_3", tyzdenIndex, refHotel);

      if (Object.keys(noveCeny).length > 0) {
        const { data: stPoAute } = await supabase
          .from("stanice")
          .update({ ...noveCeny, auto_ceny_posledne: teraz.toISOString() })
          .eq("id", st.id)
          .select()
          .single();
        st = stPoAute ?? { ...st, ...noveCeny, auto_ceny_posledne: teraz.toISOString() };
      }
      ...
      for (const b of hotoveTeraz) {
        ...
        const refB = skutocnaReferencnaCena(b.kategoria, b.typ, hDatum, globalnyMult);
        if (!refB) continue;
        const nova = autoCena(st.id, b.kategoria, b.typ + b.id, tyzdenIndex, refB);
        if (nova && nova !== b.cena) {
          await supabase.from("budovy").update({ cena: nova }).eq("id", b.id);
          b.cena = nova;
        }
      }
    }
  ...
  const spokojnostPrvy = vypocitajSpokojnostStrediska(hotoveTeraz, st, hDatum, globalnyMult, maZasnezovanie, jeBurka, jeSilnyVietor, {}, {}, "zamracene", vekDni);
  let spokojnostVysledok = spokojnostPrvy;
    const spokojnostLuka = spokojnostPrvy.spolu / 100;
 ...
 // Poradie lanovky v jej slote (na určenie, ktorá je "prvá" pre letnú prevádzku)
  const poradieVSlote = {};
  const pocitadloSlotov = {};
  for (const b of hotoveTeraz.filter((x) => x.kategoria === "lanovka").sort((a, b) => new Date(a.created_at) - new Date(b.created_at))) {
    const kluc = `${b.zona}:${b.slot || b.kategoria}`;
    poradieVSlote[b.id] = pocitadloSlotov[kluc] || 0;
    pocitadloSlotov[kluc] = (pocitadloSlotov[kluc] || 0) + 1;
  }

  for (const b of hotoveTeraz) {
    if (b.kategoria === "lanovka" && b.cena) {
      const infoB = KATEGORIE.lanovka.katalog[b.typ];
      if (jeStrediskoZatvorene || jeLanovkaMimoSezony(b, hDatum.getMonth(), poradieVSlote[b.id] ?? 0)) {
      ...
      rozpisTuristovPodBudov.push({ nazov: infoB?.nazov || b.typ, kategoria: b.kategoria, kapacita: kapacitaBudovy(b.kategoria, b.typ, b.znacka), turistiZaHodinu: Math.round(turistiSkutocneB) });
      }
    }
  }

 // Dopyt = koľko ľudí by chcelo prísť (podľa prestíže a poolu), bez ohľadu na kapacitu.
  // Kapacita lanoviek určuje, koľkých reálne odbavíš — prebytok tvorí rady.
  const zonaDopyt = {};
  for (const zonaKluc of Object.keys(kvoty)) {
    if (kvoty[zonaKluc] != null) zonaDopyt[zonaKluc] = kvoty[zonaKluc];
  }
 ...
 spokojnostVysledok = vypocitajSpokojnostStrediska(
  ...
  for (const b of hotoveTeraz) {
    ...
    const operacnaEfektivita =
      ...
      efektivnyBonus *
      konkurencnyMultiplikator(b.kategoria, b.zona, pocetKonkurencie, urovneKonkurencie) *
    ...
    rozpisTuristovPodBudov.push({ nazov: infoKapacita?.nazov || b.typ, kategoria: b.kategoria, kapacita: infoKapacita?.kapacita || 0 });
    }
...
// --- Odmena za pozvánku ---
  // Vyplatí sa, až keď pozvaný hráč naozaj hrá — 30 dní a dosť prestíže z budov.
  if (st.pozval_id && !st.pozvanka_odmenena) {
    const splnilPrestiz = sucetPrestizBudovy >= POZVANKA_PODMIENKA_PRESTIZ_BUDOVY;
    const splnilVek = vekDni >= POZVANKA_PODMIENKA_DNI;

    if (splnilPrestiz && splnilVek) {
      // Odmena pozývajúcemu
      const { data: pozyvajuci } = await supabase
        .from("stanice")
        .select("id, krystaly")
        .eq("id", st.pozval_id)
        .single();
      ...
      if (pozyvajuci) {
        await supabase
          .from("stanice")
          .update({ krystaly: (pozyvajuci.krystaly || 0) + POZVANKA_ODMENA_POZYVAJUCI })
          .eq("id", pozyvajuci.id);

        await supabase.from("spravy").insert({
          od_stanica_id: st.id,
          komu_stanica_id: pozyvajuci.id,
          predmet: "Odmena za pozvánku",
          text: `Hráč, ktorého si pozval, sa v hre udomácnil. Dostávaš ${POZVANKA_ODMENA_POZYVAJUCI} kryštálov.`,
        });
      }

      // Odmena pozvanému
      const { data: stPoOdmene } = await supabase
        .from("stanice")
        .update({
          krystaly: (st.krystaly || 0) + POZVANKA_ODMENA_POZVANY,
          pozvanka_odmenena: true,
        })
        .eq("id", st.id)
        .select()
        .single();
  ...
  const konkurencnaPrestizVypocet = konkurencnaPrestiz(pocetKonkurencie, urovneKonkurencie);
...
return {
    ...
    spokojnostRozpis: spokojnostVysledok,
  };
}
