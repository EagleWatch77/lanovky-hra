...
const globalnyMult = globalnyCenovyMultiplikator(st, hotoveTeraz);
  ...
  const spokojnostVysledok = vypocitajSpokojnostStrediska(hotoveTeraz, st, hDatum, globalnyMult, maZasnezovanie, jeBurka, jeSilnyVietor);
  ...
  for (const b of hotoveTeraz) {
    if (b.kategoria === "lanovka" && b.cena) {
      ...
      if (jeStrediskoZatvorene || jeVlekMimoSezony(b.typ, hDatum.getMonth(), b.bobova_draha)) {
        ...
        const turistiZakladneB = turistiZaHodinu(b.kategoria, b.typ, cenaSkipasu, refCenaB);
        ...
        rozpisTuristovPodBudov.push({ nazov: infoB?.nazov || b.typ, kapacita: infoB?.kapacita || 0, turistiZaHodinu: Math.round(turistiSkutocneB) });
      }
    }
  }
  ...
  for (const b of hotoveTeraz) {
    ...
    if (jeLanovkaSTuristami) {
        prijemZaHodinuB = turistiInfo[b.id].turistiSkutocneB * (st.cena_skipasu ?? 15);
