...
export function jeMedzisezona(datum) {
  ...
  if (mesiac === 4 && den >= 25 && den <= 31) return true; // 25.-31. máj (jarná)
  if (mesiac === 9 && den >= 1 && den <= 7) return true; // 1.-7. október (jesenná)
  return false;
}
...
export function sezonnyPrehlad(datum) {
  const aktualna = hranicaSezony(datum);
  const rok = aktualna.koniec.getFullYear();
  if (aktualna.typ === "leto") {
    return {
      aktualna,
  ...
  return {
    aktualna,
    medzisezona: { zaciatok: new Date(rok, 4, 25), koniec: new Date(rok, 4, 31) },
    dalsia: { typ: "leto", zaciatok: new Date(rok, 5, 1) },
  };
}
