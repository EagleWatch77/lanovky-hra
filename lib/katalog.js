...
export const OBRAZKY_LANOVIEK = {
  vlek: "/lanovky/vlek.png",
  sedacka_2: "/lanovky/pevna-lanovka.png",
  sedacka_4: "/lanovky/pevna-lanovka.png",
  sedacka_4_odp: "/lanovky/odpojitelna-lanovka.png",
  sedacka_6_odp: "/lanovky/odpojitelna-lanovka.png",
  sedacka_8_odp: "/lanovky/odpojitelna-lanovka.png",
  kabinka_8: "/lanovky/kabina.png",
  kabinka_10: "/lanovky/kabina.png",
  kabinka_15: "/lanovky/kabina.png",
  lanovka_3s: "/lanovky/s3system.png",
};

// Ktoré typy sa dajú postaviť do daného slotu
export const TYPY_PRE_SLOT = {
  vlek: ["vlek"],
  lanovka: ["sedacka_2", "sedacka_4", "sedacka_4_odp", "sedacka_6_odp", "sedacka_8_odp", "kabinka_8", "kabinka_10", "kabinka_15"],
  spojnica: ["sedacka_4_odp", "sedacka_6_odp", "sedacka_8_odp", "kabinka_8", "kabinka_10", "kabinka_15"],
  spojnica_ladovec: ["lanovka_3s"],
};
...
export const LOGA_VYROBCOV = {
  ...
  silvretta: "/logo/silvretta.png",
  apex: "/logo/apex.png",
  kristallis: "/logo/kristallis.png",
  solaris: "/logo/solaris.png",
};
...
export const PONUKA_VYROBCU = {
  ...
  apex: ["sedacka_4_odp", "sedacka_6_odp", "kabinka_8", "kabinka_10"],
  kristallis: ["sedacka_6_odp", "kabinka_10", "kabinka_15", "lanovka_3s"],
  solaris: ["sedacka_8_odp", "kabinka_15", "lanovka_3s"],
};
...
export const ZONY = {
  luka: {
    ...
    ikona: "🌾",
    poradie: 1,
    limity: { vlek: 2, lanovka: 1, spojnica_udolie: 1, spojnica_hory: 1, parkovisko: 1, bar: 1, penzion: 2, pokladna: 1 },
    popisky: { bar: "Bufet", spojnica_udolie: "Spojnica do Údolia", spojnica_hory: "Spojnica do Hôr" },
  },
  udolie: {
    nazov: "Údolie",
    ikona: "🏞️",
  ...
  hory: {
    ...
    ikona: "🌲",
    poradie: 3,
    limity: { lanovka: 2, spojnica_ladovec: 1, hotel: 1 },
...
export function jeVlekMimoSezony(typ, mesiac, maBobovuDrahu) {
  if (typ !== "vlek") return false;
  if (maBobovuDrahu) return false;
  return !jeZimnyMesiac(mesiac);
}
...
// --- Nový vzorec spokojnosti (celé stredisko, len odomknuté zóny) — 3 kategórie: Infraštruktúra, Ceny, Počasie ---
export function vypocitajSpokojnostStrediska(hotoveBudovy, st, hDatum, globalnyMult, maZasnezovanie, jeBurka, jeSilnyVietor) {
  let infrastruktura = 0;
  let ceny = 0;

  const ma = (kat, zona) => hotoveBudovy.some((b) => b.kategoria === kat && b.zona === zona);

  // --- Infraštruktúra (Lúka vždy, Údolie/Hory len ak odomknuté) ---
  if (!ma("parkovisko", "luka")) infrastruktura -= 8;
  if (!ma("bar", "luka")) infrastruktura -= 8;
  ...
  // --- Ceny (príliš vysoká cena oproti referenčnej, naprieč zónami) ---
// Skipas — jedna cena pre všetky lanovky
  const cenaSkipasu = st.cena_skipasu ?? 15;
  ...
  // Ostatné budovy s vlastnou cenou
  for (const b of hotoveBudovy) {
    if (b.kategoria === "lanovka") continue;
    if (!b.cena || !KATEGORIE[b.kategoria]?.maCenu) continue;
  ...
  const spolu = Math.max(0, Math.min(100, 100 + infrastruktura + ceny + pocasie));
