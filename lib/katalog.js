...
// --- Konkurencia (od 2. sezóny, len vybrané kategórie) ---
export const KONKURENCNE_KATEGORIE = ["bar", "parkovisko", "hotel", "servis"];
export const VEK_PRE_KONKURENCIU_DNI = 90; // cca 1 sezóna
export const PODIEL_KONKURENCIE = 0.1; // 10% dopytu natrvalo odíde ku konkurencii

export function konkurencnyMultiplikator(kategoria, stanica) {
  if (!KONKURENCNE_KATEGORIE.includes(kategoria)) return 1;
  const stlpec = "konkurencia_" + kategoria;
  return stanica && stanica[stlpec] ? 1 - PODIEL_KONKURENCIE : 1;
}
