...
export function konkurencnyMultiplikator(kategoria, stanica) {
  ...
  if (!cfg || !stanica) return 1;
  const pocet = stanica["konkurencia_" + kategoria] || 0;
...
export function konkurencnaPrestiz(stanica) {
  if (!stanica) return 0;
  let sucet = 0;
  for (const kat of Object.keys(KONKURENCIA_KONFIG)) {
    const pocet = stanica["konkurencia_" + kat] || 0;
    sucet += pocet * KONKURENCIA_KONFIG[kat].prestizBonus;
  }
  return sucet;
}
