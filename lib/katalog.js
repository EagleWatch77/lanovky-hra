...
// Vráti { revenuFaktor, wageFaktor }:
// - kratšia doba ako ideál: menej príjmu ÚMERNE (a aj nižšia efektivita/prestíž), ale plat sa NEZNIŽUJE
// - dlhšia doba ako ideál: príjem sa už nezvyšuje (tma, nikto nepríde), ale plat sa ZVYŠUJE úmerne navyše
export function prevadzkovyFaktor(hraczovaDobaHodin, idealnaDobaHodin) {
  if (!idealnaDobaHodin) return { revenuFaktor: 1, wageFaktor: 1 };
  const pomer = hraczovaDobaHodin / idealnaDobaHodin;
  if (pomer <= 1) {
    return { revenuFaktor: pomer, wageFaktor: 1 };
  }
  return { revenuFaktor: 1, wageFaktor: pomer };
}
