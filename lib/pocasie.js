...
// Búrka a silný vietor ako samostatné, "navonok" prístupné hodnoty (pre vzorec spokojnosti)
// Typ počasia dnes (slnecno, zamracene, polojasno, snezi, prsi, burka)
export function typPocasiaDnes(datum) {
  const { typDna } = urciDennyStav(datum);
  return typDna?.typ || "zamracene";
}

export function jeBurkaDnes(datum) {
  const { jeBurka } = urciDennyStav(datum);
  return jeBurka;
}

export function jeSilnyVietorDnes(datum) {
  const { jeSilnyVietor } = urciDennyStav(datum);
  return jeSilnyVietor;
}

// Zasnežovanie je citlivé na vietor rovnako ako lanovky — pri silnom vetre nefunguje (mrazivá voda sa neusadí správne)
export function zasnezovanieFunguje(datum) {
  const { jeSilnyVietor } = urciDennyStav(datum);
  return !jeSilnyVietor;
}
