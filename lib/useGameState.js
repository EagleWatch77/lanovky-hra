...
export function useGameState() {
  ...
  const [spokojnostRozpis, setSpokojnostRozpis] = useState({ infrastruktura: 0, sluzby: 0, ceny: 0, pocasie: 0, spolu: 100 });
  ...
  async function vytvorStanicu(nazov, logo, menoHraca) {
    ...
    // Kód pozvánky z adresy (?pozvanka=ABC123)
    let pozvalId = null;
    if (typeof window !== "undefined") {
      const kod = new URLSearchParams(window.location.search).get("pozvanka");
      if (kod) {
        const { data: pozyvajuci } = await supabase
          .from("stanice")
          .select("id")
          .eq("pozvanka_kod", kod.toUpperCase())
          .maybeSingle();
        if (pozyvajuci) pozvalId = pozyvajuci.id;
      }
    }

    // Vlastný kód pozvánky — 8 znakov z náhodného základu
    const vlastnyKod = Math.random().toString(36).substring(2, 10).toUpperCase();

    const { data: novaStanica } = await supabase
      .from("stanice")
      .insert({
        user_id: userId,
        nazov,
        logo: logo || "hory",
        meno_hraca: menoHraca || null,
        peniaze: 500000,
        pozvanka_kod: vlastnyKod,
        pozval_id: pozvalId,
      })
      .select()
      .single();

    // Štartovací vlek — hráč má hneď z čoho žiť, aj keď príde uprostred zimy
    if (novaStanica) {
      const info = KATEGORIE.lanovka.katalog.vlek;
   const teraz = new Date().toISOString();
      await supabase.from("budovy").insert({
        stanica_id: novaStanica.id,
        kategoria: "lanovka",
        typ: "vlek",
        znacka: "alpenlift",
        slot: "vlek",
        zona: "luka",
        stav: "hotovo",
        zaciatok_vystavby: teraz,
        koniec_vystavby: teraz,
        zamestnanci_pridelenych: info.zamestnanci,
        cena: info.referencnaCena,
      });
    }
  ...
  async function zmenitMenoHraca(novyMeno) {
    ...
    // Údaje o hráčovi do profilu
  async function ulozitProfil({ vek, bydlisko, popis, pohlavie }) {
    const zmeny = {};
    if (vek !== undefined) zmeny.vek = vek === "" || vek === null ? null : Number(vek);
    if (bydlisko !== undefined) zmeny.bydlisko = bydlisko?.trim() || null;
    if (popis !== undefined) zmeny.popis = popis?.trim().slice(0, 500) || null;
    if (pohlavie !== undefined) zmeny.pohlavie = pohlavie;
 ...
 async function postavitBudovu(kategoria, typ, znacka, zona, sBobovouDrahou = false, slot = null) {
    const slotKluc = slot || kategoria;
   ...
   if (zona === "luka" && kategoria === "lanovka") {
      const potrebnaZona = ODOMKNUTIE_LANOVIEK_LUKA[slotKluc];
      if (potrebnaZona === "udolie" && !stanica.udolie_odomknute) {
        alert("Táto spojnica sa odomkne spolu s Údolím!");
        return;
      }
      if (potrebnaZona === "hory" && !stanica.hory_odomknute) {
        alert("Táto spojnica sa odomkne spolu s Horami!");
        return;
      }
    }
    if (slotKluc === "spojnica_ladovec") {
      alert("Ľadovec zatiaľ nie je dostupný.");
      return;
    }

    const limit = ZONY[zona]?.limity[slotKluc] || 0;
  const obsadene = budovy.filter(
      (b) => b.zona === zona && b.stav !== "zrusene" && (b.slot || b.kategoria) === slotKluc
    ).length;
    if (obsadene >= limit) {
    ...
    const { data: novaBudova } = await supabase
      ...
      .insert({
        ...
        kategoria,
        typ,
       znacka: kategoria === "lanovka" ? znacka : null,
        stav: "vo_vystavbe",
  ...
  async function zmenitCenu(budova, novaCena) {
    if (budova.cena_zmenena_at) {
      ...
      if (hodinOdZmeny < CENA_COOLDOWN_HODIN) return;
    }
    const { data: updatedBud } = await supabase
      .from("budovy")
  ...
  // --- Prestavba budovy na vyšší typ (platíš plnú cenu novej, žiadne búranie) ---
  async function prestavatBudovu(budova, novyTyp) {
    if (!PRESTAVITELNE_KATEGORIE.includes(budova.kategoria)) return;
    if (budova.stav !== "hotovo") return;

    const cena = cenaBudovy(budova.kategoria, novyTyp, budova.znacka);
    if (stanica.peniaze < cena) return;

    const info = KATEGORIE[budova.kategoria].katalog[novyTyp];
    const teraz = new Date();
    ...
    setStanica(updatedSt);
    setBudovy(budovy.map((b) => (b.id === budova.id ? updatedBud : b)));
    await supabase.from("transakcie").insert({ stanica_id: stanica.id, suma: -cena, typ: "stavba", kategoria: budova.kategoria });
  }

  // --- Búranie budovy (stojí % z ceny, slot sa uvoľní) ---
  async function zburatBudovu(budova) {
    if (!BURATELNE_KATEGORIE.includes(budova.kategoria)) return;

    const cena = cenaBurania(budova.kategoria, budova.typ, budova.znacka);
    if (stanica.peniaze < cena) return;

    await supabase.from("budovy").delete().eq("id", budova.id);
    ...
    setStanica(updatedSt);
    setBudovy(budovy.filter((b) => b.id !== budova.id));
    await supabase.from("transakcie").insert({ stanica_id: stanica.id, suma: -cena, typ: "stavba", kategoria: budova.kategoria });
  }

  // --- Predaj budovy za zostatkovú cenu (ratraky) ---
  async function predatBudovu(budova) {
    if (!PREDAJNE_KATEGORIE.includes(budova.kategoria)) return;

    const povodnaCena = cenaBudovy(budova.kategoria, budova.typ, budova.znacka);
    const vekDni = (new Date() - new Date(budova.created_at)) / (1000 * 60 * 60 * 24);
    const vratene = zostatkovaCena(povodnaCena, vekDni);

    await supabase.from("budovy").delete().eq("id", budova.id);

    const { data: updatedSt } = await supabase
      .from("stanice")
      .update({ peniaze: stanica.peniaze + vratene })
      .eq("id", stanica.id)
      .select()
      .single();
    ...
    setStanica(updatedSt);
    setBudovy(budovy.filter((b) => b.id !== budova.id));
    await supabase.from("transakcie").insert({ stanica_id: stanica.id, suma: vratene, typ: "prijem", kategoria: budova.kategoria });
  }

  // Kúpa automatických cien za kryštály
  async function kupitAutoCeny(obdobie) {
    const cena = obdobie === "rok" ? AUTO_CENY_ROK_KRYSTALOV : AUTO_CENY_SEZONA_KRYSTALOV;
    if ((stanica.krystaly ?? 0) < cena) return;

    // Herná sezóna = 3 reálne mesiace, herný rok = 6 reálnych mesiacov
    const dni = obdobie === "rok" ? 182 : 91;
    const zaklad = stanica.auto_ceny_do && new Date(stanica.auto_ceny_do) > new Date()
      ? new Date(stanica.auto_ceny_do)
      : new Date();
    const platneDo = new Date(zaklad.getTime() + dni * 24 * 60 * 60 * 1000);

    const { data: updatedSt } = await supabase
      .from("stanice")
      .update({
        krystaly: (stanica.krystaly ?? 0) - cena,
        auto_ceny_do: platneDo.toISOString(),
      })
      .eq("id", stanica.id)
      .select()
      .single();
  ...
  async function zmenitCenyStrediska(nove) {
    if (stanica.ceny_zmenene_at) {
      const hodinOdZmeny = (new Date() - new Date(stanica.ceny_zmenene_at)) / (1000 * 60 * 60);
      if (hodinOdZmeny < CENA_COOLDOWN_HODIN) return;
    }
    const { data: updatedSt } = await supabase
      .from("stanice")
      .update({ ...nove, ceny_zmenene_at: new Date().toISOString() })
      .eq("id", stanica.id)
      .select()
      .single();
    setStanica(updatedSt);
