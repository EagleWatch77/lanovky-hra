...
export function useGameState() {
  ...
  async function nacitajVsetko() {
    ...
    if (hodin > 0.01 || nakladyNaNajatie > 0) {
      ...
      setZisk(zarobene);
      if (zarobene !== 0) {
        await supabase.from("transakcie").insert({ stanica_id: st.id, suma: zarobene, typ: "prevadzka" });
      }
    } else {
      setZisk(0);
    }
  ...
  async function postavitBudovu(kategoria, typ, znacka) {
    ...
    await supabase.from("transakcie").insert({ stanica_id: stanica.id, suma: -cena, typ: "stavba" });
    setLoading(false);
  }
  ...
  async function najatPreBudovu(budova) {
    ...
    await supabase.from("transakcie").insert({ stanica_id: stanica.id, suma: -CENA_NAJATIA, typ: "zamestnanec" });
  }
