...
export function useGameState() {
  ...
  async function nacitajVsetko() {
    ...
    // Konkurencia — raz za sezónu pribudne 1 jednotka v kategórii, ktorú nemáš postavenú (do limitu)
    ...
    if (vekDni >= VEK_PRE_KONKURENCIU_DNI && st.posledna_sezona_konkurencie !== sezonaTeraz) {
      const aktualizaciaKonkurencie = { posledna_sezona_konkurencie: sezonaTeraz };
      ...
      for (const kat of Object.keys(KONKURENCIA_KONFIG)) {
        ...
        const stlpecPocet = "konkurencia_" + kat;
        const aktualnyPocet = st[stlpecPocet] || 0;
        if (!mam && aktualnyPocet < cfg.max) {
          aktualizaciaKonkurencie[stlpecPocet] = aktualnyPocet + 1;
        }
      }
      const { data: stConKonk } = await supabase
        .from("stanice")
        .update(aktualizaciaKonkurencie)
        .eq("id", st.id)
        .select()
        .single();
      st = stConKonk ?? { ...st, ...aktualizaciaKonkurencie };
    }
    ...
    for (const b of hotoveTeraz) {
      const potrebnyB = zamestnanciPotrebni(b.kategoria, b.typ);
      const efektivitaB = efektivitaZamestnancov(b.zamestnanci_pridelenych || 0, potrebnyB) * efektivnyBonus * konkurencnyMultiplikator(b.kategoria, st);
    ...
    const cielovaPrestiz = Math.round(sucetPrestiz + konkurencnaPrestiz(st));
