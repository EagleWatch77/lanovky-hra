...
export function useGameState() {
  ...
  async function nacitajVsetko() {
    ...
    const { revenuFaktor: prevRevenuFaktor, wageFaktor: prevWageFaktor } = prevadzkovyFaktor(st.prevadzka_hodin ?? 7.5, idealDoba);
    for (const b of hotoveTeraz) {
      ...
      if (b.cena && KATEGORIE[b.kategoria].maCenu) {
        ...
        const referencnaCenaDnes = skutocnaReferencnaCena(b.kategoria, b.typ, hDatum, globalnyMult);
        const prijemB = prijemZaHodinu(b.kategoria, b.typ, b.cena, referencnaCenaDnes) * efektivitaB * vietorMultiplikator * hodin;
  ...
  async function zmenitPrevadzkovuDobu(noveHodiny) {
    const hodiny = Math.min(PREVADZKA_HODIN_MAX, Math.max(PREVADZKA_HODIN_MIN, noveHodiny));
    const { data: updatedSt } = await supabase
      .from("stanice")
      .update({ prevadzka_hodin: hodiny })
      .eq("id", stanica.id)
      .select()
      .single();
    setStanica(updatedSt);
