...
export function useGameState() {
  ...
  async function nacitajVsetko() {
    ...
    const jeRatrakoveObdobie = [11, 0, 1, 2, 3, 4].includes(hDatum.getMonth()); // December - Máj
  ...
  const vsetkyStanice = ligaData || [];
    const mojaLiga = vyberLiguHracov(st.id, st.created_at, vsetkyStanice);   // ← NOVÝ riadok

    function sucetEfektivnejPrestizeZona(filterFn) {
      return mojaLiga.filter(filterFn).reduce((sum, s) => {                  // ← zmenené: vsetkyStanice → mojaLiga
    ...
    for (const b of hotoveTeraz) {
      ...
      if (jeLanovkaSTuristami) {
        ...
        sucetPrestiz += turistiInfo[b.id].turistiSkutocneB * prestizNaTuristu * operacnaEfektivita;
      } else {
