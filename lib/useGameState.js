...
export function useGameState() {
  ...
  async function nacitajVsetko() {
    ...
    // Skutočný denný dopyt z lanoviek podľa zóny (podľa ceny + sezónnej krivky, nie surová kapacita)
    // — potrebné pre denný model parkoviska/ubytovania, nech reaguje na sezónu rovnako ako všetko ostatné
    const zonaDennyDav = {};
    for (const b of hotoveTeraz) {
      if (b.kategoria === "lanovka" && b.cena) {
        const refCenaLanovky = skutocnaReferencnaCena(b.kategoria, b.typ, hDatum, globalnyMult);
        const turistiZaHodinuB = turistiZaHodinu(b.kategoria, b.typ, b.cena, refCenaLanovky);
        zonaDennyDav[b.zona] = (zonaDennyDav[b.zona] || 0) + turistiZaHodinuB;
      }
    }
   ...
   if (b.kategoria === "lanovka" && b.cena) {
        const turistiB = turistiZaHodinu(b.kategoria, b.typ, b.cena, referencnaCenaDnes);
        const prestizNaTuristu = 2 + 6 * efektivitaB; // 2-8 podľa spokojnosti, 5 je stred
        sucetPrestiz += turistiB * prestizNaTuristu;
      } else {
