...
export function useGameState() {
  ...
  async function nacitajVsetko() {
    ...
    // Konkurencia — objaví sa v kategórii, ktorú nemáš postavenú, keď je stredisko dosť staré
    const vekDni = (teraz - new Date(st.created_at)) / (1000 * 60 * 60 * 24);
    if (vekDni >= VEK_PRE_KONKURENCIU_DNI) {
      const aktualizaciaKonkurencie = {};
      for (const kat of KONKURENCNE_KATEGORIE) {
        const mam = hotoveTeraz.some((b) => b.kategoria === kat);
        const stlpec = "konkurencia_" + kat;
        if (!mam && !st[stlpec]) {
          aktualizaciaKonkurencie[stlpec] = true;
        }
      }
      if (Object.keys(aktualizaciaKonkurencie).length > 0) {
