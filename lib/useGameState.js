...
async function poslatAliancnuSpravu(text) {
    if (!text.trim() || !stanica?.aliancia_id) return;
    const { error } = await supabase
      .from("aliancia_forum")
      .insert({ aliancia_id: stanica.aliancia_id, od_stanica_id: stanica.id, text: text.trim() });
    if (error) {
...
const pocetKonkurencie = {};
  ...
  const pocetNeprecitanychVoFore = stanica
    ? aliancneSpravy.filter((s) => new Date(s.created_at) > new Date(stanica.forum_precitane_do || 0)).length
    : 0;
