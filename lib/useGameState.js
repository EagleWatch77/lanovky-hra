...
export function useGameState() {
  ...
  async function nacitajVsetko() {
    ...
    // Počty DOKONČENÝCH jednotiek konkurencie na "zona:kategoria" — toto ovplyvňuje dopyt/prestíž
  ...
  async function vyjednatPlat(volba) {
    ...
    const hernyBonusDo = new Date(cielovyRok, 0, 31, 23, 59, 59); // herný "31. január" cieľového roka
    const bonusDo = realDatumZHerneho(hernyBonusDo); // prevod na reálny okamih, kedy to v hernom svete skutočne nastane
  ...
  async function poziadatOVstup(alianciaId) {
    const { error } = await supabase
      .from("aliancia_ziadosti")
      .insert({ aliancia_id: alianciaId, stanica_id: stanica.id, stav: "cakajuca" });
  ...
  async function zamietnutZiadost(ziadostId) {
  ...
  async function pozvatHraca(alianciaId, cieloveStanicaId) {
    const { error } = await supabase
      .from("aliancia_ziadosti")
      .insert({ aliancia_id: alianciaId, stanica_id: cieloveStanicaId, typ: "pozvanka", stav: "cakajuca" });
    if (error && error.code !== "23505") {
      alert("Chyba: " + error.message);
    }
  }
